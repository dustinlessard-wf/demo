...
RUN echo "Starting the script sections" && \
		...
		tar czvf demo.pub.tgz LICENSE README.md pubspec.yaml bin/ lib/ && \
		pub run semver_audit report --repo Workiva/demo && \
		curl https://codecov.workiva.net/bash > ./codecov.sh && \
		chmod a+x ./codecov.sh && \
		./codecov.sh -u https://codecov.workiva.net -t $CODECOV_TOKEN -r Workiva/demo -f coverage/coverage.lcov && \
		echo "Script sections completed"
ARG BUILD_ARTIFACTS_DOCUMENTATION=/build/doc/api/api.tar.gz
ARG BUILD_ARTIFACTS_PUB=/build/demo.pub.tgz
FROM scratch