...
#		xvfb-run -s '-screen 0 1024x768x24' pub run dart_dev coverage --no-html && \
		...
		tar czvf new_project.pub.tgz LICENSE README.md pubspec.yaml bin/ lib/ && \
		pub run semver_audit report --repo dustinlessard-wf/demo && \
#		curl https://codecov.workiva.net/bash > ./codecov.sh && \
#		chmod a+x ./codecov.sh && \
#		./codecov.sh -u https://codecov.workiva.net -t $CODECOV_TOKEN -r dustinlessard-wf/demo -f coverage/coverage.lcov && \
		echo "Script sections completed"
ARG BUILD_ARTIFACTS_DOCUMENTATION=/build/doc/api/api.tar.gz
ARG BUILD_ARTIFACTS_PUB=/build/new_project.pub.tgz
FROM scratch